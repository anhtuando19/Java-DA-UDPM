...
public class SanPhamForm extends javax.swing.JFrame {
    ...
    public void loadTable(){
        ...
        for(SanPham sp: this.sps.getList()){
    ...
    public boolean checkMa(String ma){
        ArrayList<SanPham> listSP = this.sps.getList();
    ...
    private void bt_deleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bt_deleteActionPerformed
        ...
        String id = tb_sanPham.getValueAt(row, 0).toString();
        this.sps.delete(id);
        this.loadTable();
        this.clearform();
    ...
    private void btn_prevActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_prevActionPerformed
        try {
            int row = tb_sanPham.getSelectedRow();
            if(row == 0){
                row = this.sps.getList().size() - 1;
                fillData(row);
                tb_sanPham.setRowSelectionInterval(row, row);
            }else{
                row--;
                fillData(row);
                tb_sanPham.setRowSelectionInterval(row, row);
    ...
    private void btn_nextActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_nextActionPerformed
        try {
            int row = tb_sanPham.getSelectedRow();
            if(row == this.sps.getList().size() - 1){
                row = 0;
                fillData(row);
                tb_sanPham.setRowSelectionInterval(row, row);
            }else{
    ...
    private void fillData(int row){
        lb_id.setText(tb_sanPham.getValueAt(row, 0).toString());
        txt_ma.setText(tb_sanPham.getValueAt(row, 1).toString());
        txt_ten.setText(tb_sanPham.getValueAt(row, 2).toString());
    }
    
    private SanPham addData(){
